...
def get_score(n, m, i, j):
    ...
    if i > 0 and lines[i][j] + 1 == lines[i-1][j] and dp_score[i-1][j] == 0:
        get_score(n, m, i-1, j)
        dp_score[i][j] += dp_score[i-1][j]

    if i < n - 1 and lines[i][j] + 1 == lines[i+1][j] and dp_score[i+1][j] == 0:
        get_score(n, m, i+1, j)
        dp_score[i][j] += dp_score[i+1][j]

    if j > 0 and lines[i][j] + 1 == lines[i][j-1] and dp_score[i][j-1] == 0:
        get_score(n, m, i, j-1)
        dp_score[i][j] += dp_score[i][j-1]

    if j < m - 1 and lines[i][j] + 1 == lines[i][j+1] and dp_score[i][j+1] == 0:
        get_score(n, m, i, j+1)
        dp_score[i][j] += dp_score[i][j+1]
